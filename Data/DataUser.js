...
class DataUser{
      ...
      async  getRoleAndUserAuth( role) {
        ...
        try {
          ...
          if (!querySnapshot.empty) {
            ...
            if(myPerson.data().status == 0){
              return "Acceso denegado, cuenta inhabilitada";
            }
            let personClass;
            console.log("ok");
   ...
   async setUserLogued(user){
    ...
    async registerUser(person) {
      ...
      let ress=await runTransaction(db, async() => {
          
        let n =new GenerateRandoms();
        let pas = n.generatePassword(8);
          console.log(pas);

         let res=  createUserWithEmailAndPassword(auth, person.email, pas)
