...
class DataJobRequest {
  ...
  uploadFiles = async (fileName, filePath, data) => {
    ...
    let canWeStillRegister = await this.authUsers(data.email);
    if (canWeStillRegister) {
      let h =
        await fetch(filePath)
          ...
          .then(async (blob) => {
            ...
            let n = await uploadBytes(archivoRef, blob)
              .then((snapshot) => {
              ...
              .then(async (downloadURL) => {
                ...
                let resultt = await addDoc(collectionn, myData).then(docRef => { return true; })
                return resultt;
              })
              .catch((error) => {
