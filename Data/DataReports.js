...
import { getFirestore,doc,getDoc,query,collection,where,getDocs, updateDoc, addDoc, serverTimestamp, Timestamp } from "firebase/firestore";
...
class DataReports{
  ...
  async getTotalWonByDate(startDate, endDate){
    try {
      const atentionCollection = collection(db, 'Atention');
      ...
      let myDateS = Timestamp.fromDate(startDate);
      let myDateE = Timestamp.fromDate(endDate);



      const q2 = query(atentionCollection, where('status', '==', 1), where ("date","<=",myDateE), where ("date",">=",myDateS));
      const querySnapshot2 = await getDocs(q2);
      let nAtentions =0; let totalWon =0;
          let promises2 = querySnapshot2.docs.map(async(atention)=>{
              console.log("Entroa último r");
              nAtentions++;
              totalWon= totalWon+ atention.data().aditionalCost + atention.data().serviceCurrentCost
          })
